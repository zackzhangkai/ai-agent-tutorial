...
- 他会自己先思考拆解：“我要先调用携程或航旅纵横的 API 查询航班 → 找出价格最低的一班 → 弹出确认框询问你是否同意支付 → 支付完成后自动调用企业微信或邮件接口发送假条通知”。
...
| **深度市场研报与调研** | 分析师每天耗费 3~4 小时搜集整理竞品动态与行业资讯 | Deep Research 范式：大纲规划 → 搜索抓取 → 交叉事实校验 → 报告生成 | 原需 4 小时的初稿搜集工作压缩至 **10 分钟** 内自主完成 |
| **AI 辅助编程与代码 Review** | 业务边界校验繁重，单元测试覆盖率低，人工评审耗时 | Cursor / Claude 智能体：读取代码树 → 自动化执行测试 → 定向纠错 | 代码 Review 效率提升 **40%**，边缘异常分支测试覆盖率翻倍 |
