...
2. **看输入输出确定性**：业务规则越明确（如：先查库存 → 再查运单 → 符合政策则退款），Agent 越容易通过工具编排（Tool Calling）实现接近 100% 的准确率。
...
- **规划（Planning）**：负责“先想后做”。面对一个复杂目标（如“帮我制定一份下周去东京的自由行攻略并预订机票”），大脑不会直接盲目生成答案，而是将其分解为：查日历 → 查天气 → 查航班 → 查景点 → 整合行程。
- **记忆（Memory）**：
...
很多开发者发现自己的 RAG 系统经常“答非所问”，根源在于仅使用了简单的余弦相似度检索。工业级 RAG 必须采用**切分 → 混合检索 → 重排序（Rerank）**的标准化三级链路。
